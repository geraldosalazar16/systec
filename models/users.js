    ...
    if(con){
        ...
        con.query(sql, function (err, result) {
            if (err) {
                ...
                //callback(null,result);
                //Guardar los enlaces del workflow si es insercion
                if(info.id == 0){
...
userModel.crearEnlacesDesdeTemplate = async function(info,callback){
    var enlaces = info['enlaces'];
    ...
    if(con){
        asyncForEach(enlaces,async (enlace) => {
            var sql = "INSERT INTO workflows_hojas_columnas ("+
            "ID_WORKFLOW,"+
            "ID_PROYECTO_PRIMAVERA,"+
            "ID_HOJA_SMARTSHEET,"+
            ...
            con.query(sql, function (err, result1) {
                if (err) {
                    resultado.resultado = 'Errors in bind creation'
...
userModel.crearEnlacesDesdeTemplateP = async function(info,callback){
    var enlaces = info['enlaces'];
